...
function drawSkyTexture(isNight: boolean): THREE.CanvasTexture {
 ...
 *
 * A textura é desenhada num efeito (não em useMemo) porque o desenho usa
 * Math.random() pras estrelas — uma chamada impura não pode rodar durante
 * o render.
 */
export function Backdrop({ isNight = false }: { isNight?: boolean }) {
  ...
  useEffect(() => {
    const tex = drawSkyTexture(isNight);
    // sincroniza a textura desenhada com o estado — legítimo, é o próprio
    // propósito desse effect (não dá pra desenhar em canvas durante o render).
